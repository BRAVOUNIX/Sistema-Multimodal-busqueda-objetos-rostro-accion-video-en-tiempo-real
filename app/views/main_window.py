...
from app.views.video_panel import VideoPanel
from app.views.img_rostro_window import ImgRostroWindow  # Agregar esta línea
...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        # Submenú Objetos
        accion_objetos = QAction(QIcon("assets/icons/object.png"), "&Objetos", self)
        accion_objetos.triggered.connect(lambda: self.redirigir_a("main_clase_color_gui.py"))
        menu_operacion.addAction(accion_objetos)

        # Submenú Rostro
        accion_rostro = QAction(QIcon("assets/icons/face.png"), "&Rostro", self)
        accion_rostro.triggered.connect(lambda: self.redirigir_a("main_rostro_gui.py"))
        menu_operacion.addAction(accion_rostro)

        # Submenú Acción
        accion_accion = QAction(QIcon("assets/icons/action.png"), "&Acción", self)
        accion_accion.triggered.connect(lambda: self.redirigir_a("main_st_gcn_gui.py"))
        menu_operacion.addAction(accion_accion)
        ...
        if self.profile == 0:
            ...
            # Acción Usuarios
            accion_usuarios = QAction(QIcon("assets/icons/users.png"), "&Usuarios", self)
            accion_usuarios.triggered.connect(self.show_user_management)
            menu_admin.addAction(accion_usuarios)
            
            # Acción Img. Rostro
            accion_img_rostro = QAction(QIcon("assets/icons/face.png"), "&Img. Rostro", self)
            accion_img_rostro.triggered.connect(self.show_img_rostro_management)
            menu_admin.addAction(accion_img_rostro)
            #-----
    ...
    def show_video_panel(self):
        """Muestra el panel de video con todas las funcionalidades"""
        self.video_panel = VideoPanel(self.video_controller, self.username)
    ...
    def show_user_management(self):
        ...
        # Agregar nuevo método en la clase MainWindow:
    def show_img_rostro_management(self):
       """Muestra la ventana de gestión de rostros"""
       self.img_rostro_window = ImgRostroWindow(self.username, self.profile)
       self.img_rostro_window.show()
    ...
    def redirigir_a(self, nombre_script):
        """Redirige a la aplicación correspondiente pasando el nombre de usuario"""
        import sys
        import subprocess
        import os
        from pathlib import Path
        try:
            # Determinar la ruta base según el script
            if nombre_script == "main_st_gcn_gui.py":
                ruta_base = "ST_GCN"
            else:
                ruta_base = "VISION_LLM"
        
            # Construir ruta completa
            ruta_completa = str(Path(__file__).parent.parent.parent / ruta_base / nombre_script)
            ...
            logger.error(f"Error al redirigir: {str(e)}")        